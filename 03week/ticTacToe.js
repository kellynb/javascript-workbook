...
function horizontalWin() {
  return board.some((item) => {
    return item.every(xValues) || item.every(oValues);
  }); 
}

// for verticalWin()
const containIndexValue = (num, value) => {
  const isTrue = board.every((arr1) => {
   return arr1[num] == value;
 })
  return isTrue;
}

const verticalWin = () => {
 return (containIndexValue(0, 'O') || containIndexValue(1, 'O') || containIndexValue(2, 'O')) ||  
        (containIndexValue(0, 'X') || containIndexValue(1, 'X') || containIndexValue(2, 'X'));
}
